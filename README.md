...
VibeLink 正从 Node control plane + Rust data plane 的混合架构迁移为 Rust 桌面服务。Workspace、MCP 和 Event Store 已进入 `canary`；Status、Doctor、Devices、设备令牌写操作和配对读/决策已有独立 Rust HTTP 开关。迁移期间保留 Node/Worker 回退，只有全部产品职责完成 Rust 所有权和观察窗口后才移除捆绑 Node。Audio/Compression 的旧性能 sidecar 因没有测得收益保持 `contract`，但实时通话等产品职责仍在全量迁移范围内。
...
- 大文件分页、二进制富预览和更完整的批量文件操作仍是 partial，统一在 `docs/product-status.md` 跟踪。

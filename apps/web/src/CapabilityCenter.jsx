...
import { automationDraftPayload, capabilityCategories } from "./capabilityCenterModel.js";
...
export function CapabilityCenter({ request, token }) {
  ...
  async function run(key, operation) {
    ...
    try { await operation(); await refresh(); } catch (failure) { setError(failure.message); } finally { setBusy(""); }
  }
    ...
    <section className="capability-center" aria-label="Capability center">
      ...
      <div className="capability-list" role="tabpanel">
        ...
        {items.map((item) => <div className="capability-row" key={item.id}>
          ...
          {category === "hooks" ? <p>{item.count} registered hook(s) · read-only external source</p> : null}

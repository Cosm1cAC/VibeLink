...
Use `--rust-workspace-http` with the front-door flag to let Rust own authenticated file write/delete/rename operations, Git status/diff, per-file and repository Git actions, and worktree-list reads. Git actions preserve dry-run behavior, tool-run lifecycle events, and success/failure audits, including bounded single-file hunk staging. The routes enforce SQLite workspace roots, bounded bodies/Git output, path traversal checks, untracked text preview limits, and audit/update contracts; unsupported workspace and tool routes continue to fall back to Node.

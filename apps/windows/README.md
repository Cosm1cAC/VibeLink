...
Use `--rust-workspace-http` with the front-door flag to let Rust own authenticated `POST /api/workspaces/:id/file` write, delete, and rename operations plus Git status/diff and worktree-list reads. The routes enforce SQLite workspace roots, bounded bodies/Git output, path traversal checks, untracked text preview limits, and audit/update contracts; unsupported workspace and tool routes continue to fall back to Node.

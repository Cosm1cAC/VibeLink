...
impl WorkspaceRouteConfig {
    pub fn new(data_dir: PathBuf) -> Self {
        Self { data_dir, metrics: Arc::new(WorkspaceRouteMetrics::default()) }
    }

    pub(crate) fn record_fallback(&self) { self.metrics.fallbacks.fetch_add(1, Ordering::SeqCst); }
}

pub fn workspace_request_requires_body(request: &ParsedRequest) -> bool {
    request.method == "POST" && request.path().starts_with("/api/workspaces/") && request.path().ends_with("/file")
}
...
pub fn route_workspace_request(
    ...
    let Some((workspace_id, action_path)) = workspace_path_parts(request.path()) else { return Ok(None); };
    if request.method != "POST" || action_path != "file" { return Ok(None); }
    let auth = authenticate_route_request(request, &config.data_dir)?;
    if auth == RouteAuthentication::Pending { return Ok(None); }
    ...
    if body.len() > MAX_BODY_BYTES { return Ok(Some(HttpRouteResponse::error(413, "Request body is too large."))); }
    let payload: Value = serde_json::from_slice(body).context("Invalid workspace file JSON")?;
    let action = payload.get("action").and_then(Value::as_str).unwrap_or("write").to_ascii_lowercase();
    let result = mutate_workspace(&config.data_dir, &workspace_id, &action, &payload)?;
    config.metrics.responses.fetch_add(1, Ordering::SeqCst);
    Ok(Some(HttpRouteResponse::json(200, result)))
}
...
fn mutate_workspace(data_dir: &Path, workspace_id: &str, action: &str, payload: &Value) -> Result<Value> {
    let db_path = data_dir.join("mobile-agent.sqlite");
    let connection = Connection::open_with_flags(&db_path, OpenFlags::SQLITE_OPEN_READ_WRITE | OpenFlags::SQLITE_OPEN_NO_MUTEX)
        .with_context(|| format!("Cannot open {}", db_path.display()))?;
    connection.busy_timeout(Duration::from_secs(5))?;
    let (root, title) = connection.query_row(
        "SELECT path, title FROM workspaces WHERE id = ?1",
        params![workspace_id],
        |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?)),
    ).optional()?.ok_or_else(|| anyhow::anyhow!("Workspace not found."))?;
    ...
    let result = match action {
        "write" => {
            ...
            if text.as_bytes().len() > MAX_BODY_BYTES { bail!("Workspace file text is too large."); }
            if target.exists() && !target.is_file() { bail!("Workspace file path must be a file."); }
            if let Some(parent) = target.parent() { fs::create_dir_all(parent)?; }
            fs::write(&target, text.as_bytes())?;
            file_result(&root, &target, workspace_id, &title, "write")?
        }
        "delete" => {
            if !target.is_file() { bail!("Workspace file path must be a file."); }
            fs::remove_file(&target)?;
            json!({"ok":true,"action":"delete","workspace":{"id":workspace_id,"title":title},"path":relative(&root, &target)})
        }
        "rename" => {
            if !target.exists() { bail!("Workspace file path does not exist."); }
            let next = safe_child(&root, payload.get("nextPath").and_then(Value::as_str).unwrap_or(""))?;
            if next.exists() { bail!("Workspace destination already exists."); }
            if let Some(parent) = next.parent() { fs::create_dir_all(parent)?; }
    ...
    let now = format!("{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_secs());
...
fn canonical_root(path: &Path) -> Result<PathBuf> {
    let root = fs::canonicalize(path).with_context(|| format!("Workspace root does not exist: {}", path.display()))?;
    if !root.is_dir() { bail!("Workspace root is not a directory."); }
    Ok(root)
}

fn safe_child(root: &Path, value: &str) -> Result<PathBuf> {
    if value.trim().is_empty() { bail!("Workspace file path is required."); }
    let candidate = Path::new(value);
    if candidate.is_absolute() || candidate.components().any(|item| matches!(item, Component::ParentDir | Component::RootDir | Component::Prefix(_))) {
    ...
    if let Some(existing) = target.parent().filter(|path| path.exists()) {
        ...
        if !parent.starts_with(root) { bail!("Path is outside workspace."); }
    }
    Ok(target)
}

fn relative(root: &Path, target: &Path) -> String { target.strip_prefix(root).unwrap_or(target).to_string_lossy().replace('\\', "/") }

fn file_result(root: &Path, target: &Path, workspace_id: &str, title: &str, action: &str) -> Result<Value> {
    let metadata = fs::metadata(target)?;
    Ok(json!({"ok":true,"action":action,"workspace":{"id":workspace_id,"title":title},"path":relative(root,target),"size":metadata.len(),"text":String::from_utf8_lossy(&fs::read(target)?).to_string()}))
}
...
mod tests {
    ...
    fn fixture() -> (PathBuf, WorkspaceRouteConfig, String) {
        let dir = std::env::temp_dir().join(format!("vibelink-workspace-http-{}-{}", std::process::id(), SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()));
        fs::create_dir_all(&dir).unwrap();
        let root = dir.join("workspace"); fs::create_dir_all(&root).unwrap();
        ...
        db.execute("INSERT INTO workspaces VALUES ('w',?1,'Workspace',?1,'now','now',NULL)", params![root.to_string_lossy()]).unwrap();
        (dir.clone(), WorkspaceRouteConfig::new(dir), root.to_string_lossy().to_string())
    }
    ...
    fn writes_renames_and_deletes_only_inside_workspace() {
        ...
        let written = route_workspace_request(&request, Some(br#"{"path":"src/a.txt","text":"hello"}"#), &config).unwrap().unwrap();
        assert_eq!(written.status, 200);
        let renamed = route_workspace_request(&request, Some(br#"{"action":"rename","path":"src/a.txt","nextPath":"src/b.txt"}"#), &config).unwrap().unwrap();
        assert_eq!(renamed.status, 200);
        let deleted = route_workspace_request(&request, Some(br#"{"action":"delete","path":"src/b.txt"}"#), &config).unwrap().unwrap();
        assert_eq!(deleted.status, 200);
        assert!(route_workspace_request(&request, Some(br#"{"path":"../escape","text":"x"}"#), &config).is_err());

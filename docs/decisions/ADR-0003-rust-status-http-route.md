...
4. Node exposes `GET /internal/status-snapshot` only when the peer is loopback and `X-VibeLink-Internal-Token` matches a 32-byte OS-random process credential using constant-time comparison. Rust passes this credential through the child environment, never through argv.
5. The internal endpoint returns the existing dynamic snapshot before Status rendering. Rust validates and renders it with the same `status_sidecar` contract, then returns the existing JSON content type, no-store cache policy, and status code.

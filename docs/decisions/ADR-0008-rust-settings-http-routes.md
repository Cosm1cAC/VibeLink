...
- After a Rust mutation, call loopback-only `POST /internal/reload-settings` with the random internal-control token. This endpoint may reload and normalize the Node in-memory copy during the hybrid phase, but it is not a public management API and does not own validation, persistence intent, credentials, or audit policy.

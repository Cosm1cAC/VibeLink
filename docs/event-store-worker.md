...
The Windows event-store workflow repeats contract/runtime tests plus local, runtime, and server canaries every Monday at 05:17 UTC. Canary JSON artifacts are retained for 30 days. This automates recurring remote evidence without treating synthetic/server traffic as the outstanding human-driven append session.

...
The Codex app-server adapter is enabled only when `npm run codex-app-server:contract` succeeds for a reviewed CLI minor. The reviewed allowlist contains `0.117` and `0.144`; the checked 0.144 fixture was generated by Codex CLI 0.144.5. Missing lifecycle/output/approval methods, tool item discriminators, response fields, params-schema drift, invalid generator output, or an unreviewed minor disables the complete adapter.

`provider.appServer` starts use backend `app_server` and include strict `appServer.threadResumeParams` and `appServer.turnStartParams` objects with the same non-empty `threadId`. The worker probes the CLI minor before spawning, starts a loopback-only WebSocket app-server, and performs `initialize`, `initialized`, `thread/resume`, and `turn/start` in order. Reviewed notifications become durable `provider.event` records; connection closure terminates the owned Job and produces an authoritative `execution.exited`. Command, file-change, and permission approval requests retain their connection-scoped JSON-RPC identity in the worker. Transactional outbox delivery responds to that original request and follows durable worker events to `delivered`, `applied`, or `stale`, including after a Bridge restart.

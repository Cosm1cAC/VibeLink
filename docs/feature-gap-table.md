...
- Android 原生端已从 MVP 升级为主要移动客户端：会话管理、VibeLink Agent composer、Codex Desktop Remote、Workspace/Git/Test/Terminal、Settings/Approvals/admin、Tool events、附件/分享、原生通知策略和 Live Call Assistant 均已有原生入口并通过 debug 构建；闭环状态见 `docs/android-parity-closure-report.md`。
...
- Markdown 渲染、图片查看、消息操作、Workspace 文件树、链接路径定位、PTY 终端、Git 深水区操作、测试输出视图和 Android 移动端入口已经有可用实现，但仍保留部分 Codex/平台级差异项。
...
| 状态 | 差异点 | 当前情况 | 优先级 |
| --- | --- | --- | --- |
| partial | change card | 前端已有变更卡、diff viewer、文件级接受/拒绝、全部暂存和 task/workspace 变更摘要；Android 已补 per-hunk stage 和冲突动作。仍缺评论、历史快照对比和 Codex 原生 change set 语义。 | P1 |
| partial | 文件树 | 已有 workspace 文件树、目录导航、文本文件打开、文件搜索、文本编辑、新建、重命名和删除；仍缺大文件分页、富二进制预览和更完整的批量文件操作。 | P1 |
| partial | 文件定位 | Markdown 链接、本地文件链接和可解析路径可跳到对应 workspace 文件与行号并高亮；仍依赖路径解析和已登记 workspace，裸路径自动识别、编译器/测试框架路径格式覆盖还不完整。 | P1 |
| partial | 内置终端 | 已有 workspace 命令运行器和 Android PTY terminal session，可显示 stdout/stderr/退出状态，支持输入、resize、stop 和输出轮询，并接入危险命令逐条确认；仍缺任务树、ANSI 完整渲染和长期 shell 会话产品化打磨。 | P1 |
| partial | Git 完整工作流 | 已支持 status/diff、stage/unstage、stage all、commit、push、pull、`gh pr create`、branch create/switch、stash push/pop、永久 worktree 创建、per-hunk stage 和冲突动作；仍缺 branch 删除、worktree 列表/移除、PR review 和远端凭据引导。 | P1 |
| partial | 测试结果视图 | 已有测试命令入口、结构化 passed/failed 粗解析、失败行提取、日志折叠，并在 Android Workspace 暴露测试执行/结果入口；仍缺 Jest/Pytest/Vitest 等框架适配、失败跳转、重跑单测和耗时趋势。 | P1 |
...
| 状态 | 差异点 | 当前情况 | 优先级 |
| --- | --- | --- | --- |
| partial | 原生移动端 | Android 已有原生登录/配对、会话搜索/归档/置顶/重命名/fork、VibeLink Agent composer、新建/继续/停止任务、Codex Remote 按需同步和发送队列控制、Workspace 文件/Git/Test/PTY、Settings/Approvals/admin、Tool events 卡片、附件/系统分享、通知/后台策略和 Live Call Assistant。仍缺 iOS App，以及更长时间弱网和消费级交互打磨。 | P1 |
...
| partial | 推送通知 | 已有 Web Push 订阅、关键事件通知入口、邮件 fallback 配置，以及 Android FCM credential 配置、native push token 注册和服务端投递路径；仍缺 iOS APNs/Expo Push、通知偏好细分和投递状态看板。 | P0 |
| partial | 断线恢复 | task events、按需产生的 desktop observations、live call events 均可写入 SQLite；任务和 live call 仍有 SSE/catch-up，Android 已有弱网 polling、后台 catch-up 和多设备同步策略测试。仍缺 ack、保留策略、压缩和更完整的跨设备一致性测试。 | P0 |
...
| partial | 设置页 | 设置页可配置模型命令、权限、安全、公网、凭据状态、设备/配对、audit、MCP、Doctor、Cloudflare、tool retention、push 和 settings import/export；仍缺更成熟的配置中心层级、风险解释和配置验证报告。 | P1 |
...
| partial | 暗色模式 | Web/Android 已支持暗色体验，Android 跟随系统主题；仍需继续做细节对比度、复杂消息块和设计令牌一致性打磨。 | P2 |
| missing | 命令面板/快捷键 | 没有快捷键、命令面板、键盘导航体验。 | P2 |
| partial | 移动适配 | Android 原生端已补齐系统分享、文件/图片选择、通知/后台策略、弱网恢复决策和 48dp 触控目标等关键项；仍需长时间手机操作、复杂列表密度和消费级微交互打磨。 | P1 |
...
| partial | 多设备同步 | 多设备可凭 device token 登录并共享事件流，Android 已有多设备同步状态策略；仍缺在线状态、冲突处理、消息操作同步和设备间协作策略。 | P1 |
| partial | 平台定位 | 当前仍是本机 bridge + Web/Android 客户端，不是完整公网安全远程 agent 平台；iOS、部署体检和账号级安全策略仍未产品化，Android 原生通知和后台策略已进入可用闭环。 | P0 |
...
3. `P1`：继续打磨 PTY 会话：ANSI 完整渲染、长期 shell 会话、任务树、停止语义和日志折叠。
4. `P1`：继续完善 Workspace/Git：裸路径自动链接、worktree 列表/移除、PR review、远端凭据引导和冲突向导。

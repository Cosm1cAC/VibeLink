...
| Workspace 文件与 Git Status 原生 HTTP 路由 | `default-on` | Rust allowed-root 文件操作与 Git status，Node 回退 | 继续迁移 Git diff/actions/worktree/command/approval |
...
同一默认开启的 route family 现已接管 `GET /api/workspaces/:id/git/status`。Rust 使用参数化 workspace 查询和 canonical 工作目录执行固定参数 `git status --porcelain=v1 -b`，保持 branch/files/changedCount/stdout/stderr/exitCode/workspace/cwd 合同、10 MiB 输出上限，以及 Node 的 workspace 最近使用时间更新语义。Git diff、写 action、worktree、command 与 approval continuation 仍由 Node 持有。

当前本地 111 项 Rust 测试覆盖文件写入、重命名、删除、Git status 合同、路径穿越拒绝和现有控制面回归。该切片已纳入默认 `vibelink.exe` Rust 前门 profile；其余 Git/command/approval 仍由 Node 负责，直到后续 workspace/tool slices 完成。

...
本文是 Rust 迁移唯一的人工维护报告。机器可读状态保存在 `docs/rust-migration-status.json`，架构决策记录在 `docs/decisions/ADR-0001` 至 `ADR-0006`。
...
| Devices 只读原生 HTTP 路由 | `opt-in` | Rust Host/鉴权/SQLite 查询/字段过滤，设备写操作仍由 Node 负责 | 远端 CI 与公网认证 canary 后晋级 |
| Devices 审计化写操作原生 HTTP 路由 | `opt-in` | Rust 令牌轮换/吊销、限流和事务化审计 | 远端 CI、便携包和公网受控 canary 后晋级 |
| Pairing 查询与决策原生 HTTP 路由 | `opt-in` | Rust 公开状态、认证列表、批准/拒绝；Node 暂留 create/claim | 增加有界 body 后迁移 create/claim |
...
`--rust-devices-http` 仅在 Rust HTTP 前门开启时生效。Rust 对精确的 `GET /api/devices` 复用 Status 的有界解析、Host allowlist 和 SQLite 设备鉴权，随后以只读连接直接查询公开设备字段，计算过期状态，并保持 `currentDeviceId`、活动时间排序、`meta` JSON 与嵌套 `fields` 过滤契约。查询不读取 `token_hash`；配对、撤销和令牌轮换仍由 Node 处理。
...
创建和领取需要有界 JSON body、QR 生成、一次性 code/token 以及 public settings 投影，本阶段仍逐字节转发 Node。真实 release canary 完成 Node create、Rust pending status/list/approve、Node claim 和 Rust claimed status；批准与领取审计均可连续回查，Rust/Node 所有权头严格落在计划路由。前门可选路由同时重构为 `FrontdoorRoutes` 配置对象，避免后续迁移继续扩张函数参数列表。
...
5. 设备与配对：设备写操作已进入 `opt-in`；下一切片迁移配对 create/status/claim/approve/deny。

...
| Devices 只读原生 HTTP 路由 | `opt-in` | Rust Host/鉴权/SQLite 查询/字段过滤；写操作由独立 Rust slice 接管 | 远端 CI 与公网认证 canary 后晋级 |
...
| Settings 原生 HTTP 路由 | `opt-in` | Rust 校验/dry-run/导入导出/原子写入/DPAPI/审计；Node 仅做受保护内存重载 | 远端 CI、便携包与公网拒绝路径后保持观察 |
...
本地 71 项 Rust 单测全绿；完整 release HTTP canary 累计通过 38 项检查，其中 Settings 覆盖匿名 `401`、无 secret 导出、无落盘 dry-run、DPAPI 更新、导入预览/提交和三类成功审计。该 route family 当前保持 `opt-in`，等待远端 CI、可分发包与公网证据。
...
5. 控制面路由：设备、配对、审计与设置均已进入 `opt-in`；下一切片迁移 workspace/registry/MCP/tool-events。

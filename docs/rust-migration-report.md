...
| Workspace 文件与 Git 只读原生 HTTP 路由 | `default-on` | Rust allowed-root 文件操作与 Git status/diff，Node 回退 | 继续迁移 Git actions/worktree/command/approval |
...
同一默认开启的 route family 现已接管 `GET /api/workspaces/:id/git/status` 与 `/git/diff`。Rust 使用参数化 workspace 查询和 canonical 工作目录执行固定 Git 参数，保持 branch/files/changedCount/stdout/stderr/exitCode/workspace/cwd 合同、10 MiB 输出上限，以及 Node 的 workspace 最近使用时间更新语义。Diff 路由还保持未初始化仓库的 HEAD fallback、最多 6 个未跟踪文本预览、512 KiB 单文件预览上限和文件级增删行统计。Git 写 action、worktree、command 与 approval continuation 仍由 Node 持有。

当前本地 113 项 Rust 测试覆盖文件写入、重命名、删除、Git status/diff 合同、未初始化仓库、路径穿越拒绝和现有控制面回归。该切片已纳入默认 `vibelink.exe` Rust 前门 profile；其余 Git/command/approval 仍由 Node 负责，直到后续 workspace/tool slices 完成。

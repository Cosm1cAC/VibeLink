...
本文是 Rust 迁移唯一的人工维护报告。机器可读状态保存在 `docs/rust-migration-status.json`，架构决策记录在 `docs/decisions/ADR-0001` 至 `ADR-0004`。
...
2026-07-14 的本地 release canary 完成代理登录、Rust 匿名拒绝、认证设备列表和 `fields=id,label` 过滤，返回当前设备且所有直接响应均带 Rust 所有权头；`controlPlaneRuntime.devicesHttp` 为 3/3 响应，failure、fallback、pending 均为 0。配置、数据库或映射错误会由前门逐字节回放原请求给 Node。该 slice 当前为 `opt-in`，待远端 CI 与公网认证成功响应通过后晋级。

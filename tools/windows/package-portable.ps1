...
[IO.File]::WriteAllText(
  (Join-Path $stageRoot "start-vibelink-tool-events-sse-canary.cmd"),
  "@echo off`r`ncd /d %~dp0`r`nvibelink.exe --rust-canary --rust-http-canary --rust-status-http --rust-doctor-http --rust-devices-http --rust-device-mutations-http --rust-pairing-http --rust-audit-http --rust-settings-http --rust-tool-events-http --rust-tool-events-sse %*`r`n",
  [Text.Encoding]::ASCII
)
